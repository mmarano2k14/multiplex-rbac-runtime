...
namespace Multiplexed.AI.Runtime.AI.Rag.Providers.Connectors.SqlServer
{
    /// <summary>
    /// Relational connector for SQL Server-backed RAG reads.
    ///
    /// PURPOSE:
    /// - Resolves and executes SQL Server-specific relational queries.
    ...
    /// </summary>
    public sealed class SqlServerRelationalRagConnector : IRelationalRagConnector
    {
        private readonly IReadOnlyDictionary<string, IRelationalRagQuery> _queries;

        /// <summary>
        /// Initializes a new instance of the <see cref="SqlServerRelationalRagConnector"/> class.
        /// </summary>
        /// <param name="queries">
        /// The available relational queries.
        /// </param>
        public SqlServerRelationalRagConnector(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!_queries.TryGetValue(entityType, out var query))
            {
                throw new InvalidOperationException(
                    $"No SQL Server relational RAG query is registered for entity type '{entityType}'.");
            }
